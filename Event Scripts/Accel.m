...
function [] = AccelDisp(data,t)
    ...
    ax1 = nexttile;
    plot(time,data.GPSSpeed); grid on; hold on;
    plot(time,data.DriveSpeed)
    ...
    plot(time,slip); grid on;
    setAxesZoomConstraint(z,ax2,'x');
    ylabel('Slip'); ylim([0 max(slip)+5]);
    
    ax3 = nexttile;
    plot(time,data.EngineRPM); grid on; hold on;
    plot(time,data.LaunchAimRPM);
